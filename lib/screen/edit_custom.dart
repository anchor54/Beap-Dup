...
import '../view_model/edit_custom_vm.dart';

class EditCustom extends BaseWidget<EditCustomVM> {
  const EditCustom({super.key});

  @override
  Widget buildUI(BuildContext context, EditCustomVM viewModel) {
    return Scaffold(
                            ...
                            itemBuilder: (context, index) {
                              return InkWell(
                                  onTap: () {
                                    ...
                                    viewModel.every =
                                        "${HiveUtils.getSession<String>(SessionKey.editEveryType, defaultValue: "").isNotEmpty ? HiveUtils.getSession<String>(SessionKey.editEveryType) : "1"}${" "}${viewModel.frequency == "Daily" ? "Day" : viewModel.frequency == "Weekly" ? "week" : viewModel.frequency == "Monthly" ? "month" : ""}";
                                    viewModel.occur =
                                        "${HiveUtils.getSession<String>(SessionKey.editEveryType, defaultValue: "").isNotEmpty ? HiveUtils.getSession<String>(SessionKey.editEveryType) : "1"}${" "}${viewModel.frequency == "Daily" ? "day" : viewModel.frequency == "Weekly" ? "week" : viewModel.frequency == "Monthly" ? "month" : ""}";
                                    viewModel.tempOccur =
                                        "${HiveUtils.getSession<String>(SessionKey.editEveryType, defaultValue: "").isNotEmpty ? HiveUtils.getSession<String>(SessionKey.editEveryType) : "1"}${" "}${viewModel.frequency == "Daily" ? "day" : viewModel.frequency == "Weekly" ? "week" : viewModel.frequency == "Monthly" ? "month" : ""}";
                      ...
                      child: Container(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            return InkWell(
                                onTap: () {
                                  // Toasty.showtoast(viewModel.Weeks[index].status);
                                  // Toasty.showtoast(viewModel.Weeks[index].id.toString());
